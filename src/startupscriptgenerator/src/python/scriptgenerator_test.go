...
func ExamplePythonStartupScriptGenerator_getCommandFromModule_onlyModule() {
	...
	fmt.Println(command)
	// Output:
	// GUNICORN_CMD_ARGS="--timeout 600" gunicorn module.py
}
...
func ExamplePythonStartupScriptGenerator_getCommandFromModule_moduleAndPath() {
	...
	fmt.Println(command)
	// Output:
	// GUNICORN_CMD_ARGS="--timeout 600 --chdir=/a/b/c" gunicorn module.py
}
...
func ExamplePythonStartupScriptGenerator_getCommandFromModule_moduleAndPathAndHost() {
	...
	fmt.Println(command)
	// Output:
	// GUNICORN_CMD_ARGS="--timeout 600 --bind=0.0.0.0:12345 --chdir=/a/b/c" gunicorn module.py
}
