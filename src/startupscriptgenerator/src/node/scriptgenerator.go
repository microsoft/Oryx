...
func (gen *NodeStartupScriptGenerator) GenerateEntrypointScript() string {
	...
	if startupCommand != "" {
		userStartupCommandFullPath = filepath.Join(gen.getAppPath(), startupCommand)
	}
	commandSource := "User"
	...
	if startupCommand == "" || common.FileExists(userStartupCommandFullPath) {
		...
		packageJsonObj, packageJsonPath := getPackageJsonObject(gen.getAppPath(), userStartupCommandFullPath)
		...
		if startupCommand == "" && userStartupCommandFullPath != "" {
			...
			startupCommand = common.ExtendPathForCommand(gen.UserStartupCommand, gen.getAppPath())
			commandSource = "UserScript"
		}

		if startupCommand == "" {
			startupCommand = gen.getCandidateFilesStartCommand(gen.getAppPath())
		...
		startupCommand = common.ExtendPathForCommand(startupCommand, gen.getAppPath())
	}
...
func (gen *NodeStartupScriptGenerator) getPackageJsonStartCommand(packageJsonObj *packageJson, packageJsonPath string) string {
	if packageJsonObj != nil && packageJsonObj.Scripts != nil && packageJsonObj.Scripts.Start != "" {
		...
		isPackageJsonAtRoot := filepath.Clean(packageJsonDir) == filepath.Clean(gen.getAppPath())
		yarnLockPath := filepath.Join(gen.getAppPath(), "yarn.lock") // TODO: consolidate with Microsoft.Oryx.BuildScriptGenerator.Node.NodeConstants.YarnLockFileName
...
func (gen *NodeStartupScriptGenerator) getPackageJsonMainCommand(packageJsonObj *packageJson, packageJsonPath string) string {
	if packageJsonObj != nil && packageJsonObj.Main != "" {
		...
		if filepath.Clean(packageJsonDir) != filepath.Clean(gen.getAppPath()) {
			subPath := common.GetSubPath(gen.getAppPath(), packageJsonDir)
...
func (gen *NodeStartupScriptGenerator) getUserProvidedJsFileCommand(fileFullPath string) string {
	...
	if strings.HasSuffix(fileFullPath, ".js") {
		subPath := common.GetSubPath(gen.getAppPath(), fileFullPath)
...
func (gen *NodeStartupScriptGenerator) getCandidateFilesStartCommand(appPath string) string {
	...
	for _, file := range filesToSearch {
		fullPath := filepath.Join(gen.getAppPath(), file)
...
func (gen *NodeStartupScriptGenerator) checkStartupFileWithPath(configFileSuffix string, configFileName string, userInputFullPath string) string {
	...
	if userInputFullPath != "" {
		...
		configFilePath = filepath.Join(gen.getAppPath(), configFileName)
	}
...
func (gen *NodeStartupScriptGenerator) getAppPath() string {
