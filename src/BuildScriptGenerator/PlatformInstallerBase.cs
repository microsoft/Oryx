...
namespace Microsoft.Oryx.BuildScriptGenerator
{
    public abstract class PlatformInstallerBase
    {
        ...
        /// This is the ACR equivalent of <see cref="GetInstallerScriptSnippet"/>.
        /// No external tools (docker, crane, oras) are required.
        /// </summary>
        protected string GetAcrInstallerScriptSnippet(
            string platformName,
            string version,
            string directoryToInstall = null)
        {
            ...
            var versionDir = directoryToInstall;
            if (string.IsNullOrEmpty(versionDir))
            {
                versionDir = Path.Combine(this.CommonOptions.DynamicInstallRootDir, platformName, version);
            }
            ...
            snippet
                ...
                .AppendLine("PLATFORM_SETUP_START=$SECONDS")
                .AppendLine("echo")
                .AppendLine($"echo \"Downloading and extracting '{platformName}' version '{version}' from ACR to '{versionDir}'...\"")
                .AppendLine($"rm -rf {versionDir}")
                .AppendLine($"mkdir -p {versionDir}")
                .AppendLine($"cd {versionDir}")
                .AppendLine()
                .AppendLine("# Step 1: Get OCI manifest from ACR")
                .AppendLine("ACR_DOWNLOAD_START=$SECONDS")
                .AppendLine($"MANIFEST=$(curl -sL --connect-timeout 10 -H 'Accept: application/vnd.oci.image.manifest.v1+json' \\")
                .AppendLine($"  '{registryUrl}/v2/{repository}/manifests/{tag}')")
                .AppendLine()
                .AppendLine("# Step 2: Extract layer digest (single-layer FROM scratch image)")
                .AppendLine("LAYER_DIGEST=$(echo \"$MANIFEST\" | grep -oP '\"digest\"\\s*:\\s*\"sha256:[a-f0-9]+\"' | head -1 | grep -oP 'sha256:[a-f0-9]+')")
                .AppendLine()
                .AppendLine("if [ -z \"$LAYER_DIGEST\" ]; then")
                .AppendLine($"  echo \"ERROR: Failed to parse layer digest from ACR manifest for {platformName} {version}.\"")
                .AppendLine("  exit 1")
                .AppendLine("fi")
                .AppendLine()
                .AppendLine("# Step 3: Download layer blob (this IS the SDK tarball)")
                .AppendLine($"curl -SL --connect-timeout 10 '{registryUrl}/v2/{repository}/blobs/'$LAYER_DIGEST --output sdk-layer.tar.gz >/dev/null 2>&1")
                .AppendLine("ACR_DOWNLOAD_ELAPSED=$(($SECONDS - $ACR_DOWNLOAD_START))")
                .AppendLine("echo \"Download from ACR done in $ACR_DOWNLOAD_ELAPSED sec(s).\"")
                .AppendLine()
                .AppendLine("# Step 4: Verify SHA256 digest (built into OCI — no separate checksum metadata needed)")
                .AppendLine("echo Verifying checksum...")
                .AppendLine("EXPECTED_SHA=$(echo $LAYER_DIGEST | cut -d: -f2)")
                .AppendLine("ACTUAL_SHA=$(sha256sum sdk-layer.tar.gz | cut -d' ' -f1)")
                .AppendLine("if [ \"$EXPECTED_SHA\" != \"$ACTUAL_SHA\" ]; then")
                .AppendLine("  echo \"ERROR: SHA256 digest mismatch. Expected: $EXPECTED_SHA, Actual: $ACTUAL_SHA\"")
                .AppendLine("  exit 1")
                .AppendLine("fi")
                .AppendLine("echo Checksum verified.")
                .AppendLine()
                .AppendLine("# Step 5: Extract")
                .AppendLine("echo Extracting contents...")
                .AppendLine("tar -xzf sdk-layer.tar.gz -C .")
                .AppendLine("rm -f sdk-layer.tar.gz")
                .AppendLine()
                .AppendLine($"echo > {Path.Combine(versionDir, SdkStorageConstants.SdkDownloadSentinelFileName)}")
                .AppendLine("PLATFORM_SETUP_ELAPSED=$(($SECONDS - $PLATFORM_SETUP_START))")
                .AppendLine($"echo \"Done in $PLATFORM_SETUP_ELAPSED sec(s). Installed '{platformName}' version '{version}' from ACR.\"")
                .AppendLine();
