...
namespace Microsoft.Oryx.BuildScriptGenerator.Tests.Python
{
    public class PythonBashBuildSnippetTest
    {
        ...
        public void GeneratedSnippet_DisablePipUpgradeFlag_IfPipUpgradeFlag_IsNull()
        {
            ...
            Assert.NotEmpty(text);
            Assert.NotNull(text);
            Assert.Contains("$python -m pip install --cache-dir $PIP_CACHE_DIR --prefer-binary -r $REQUIREMENTS_TXT_FILE --target=" + snippetProps.PackagesDirectory + " " + snippetProps.PipUpgradeFlag + " | ts $TS_FMT", text);
        }
    }
}
