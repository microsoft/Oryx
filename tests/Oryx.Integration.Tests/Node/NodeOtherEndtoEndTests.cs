...
namespace Microsoft.Oryx.Integration.Tests
{
    public class NodeOtherEndtoEndTests : NodeEndToEndTestsBase
    {
        ...
        [Fact]
        [Trait("category", "node-14-5")]
        public async Task CanBuildAndRun_NodeApp_WithCustomizedRunCommand()
        {
            ...
            try
            {
                var manifestDirVolume = DockerVolume.CreateMirror(manifestDirPath,true);
                var tmpContainerDir = manifestDirVolume.ContainerDir;
                var volume = CreateAppVolume(appName);
                var appDir = volume.ContainerDir;
                var appOutputDirVolume = CreateAppOutputDirVolume();
                var appOutputDir = appOutputDirVolume.ContainerDir;
                ...
                var buildImageScript = new ShellScriptBuilder()
                   ...
                   .AddCommand(
                    $"oryx build {appDir} -i /tmp/int --platform {NodeConstants.PlatformName} " +
                    $"--platform-version {nodeVersion} -o {appOutputDir}")
                   .ToString();
                ...
                await EndToEndTestHelper.BuildRunAndAssertAppAsync(
                    appName,
                    _output,
                    new DockerVolume[] { volume, appOutputDirVolume, manifestDirVolume },
                    ...
                    async (hostPort) =>
                    {
                        ...
                        var runScript = File.ReadAllText(Path.Combine(manifestDirPath, "run.sh"));
                        Assert.Contains(runCommand, runScript);
                    });
            }
            finally
            {
                Directory.Delete(manifestDirPath, true);
            }
        }
    }
}