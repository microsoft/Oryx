...
namespace Microsoft.Oryx.Integration.Tests
{
    ...
    public class DotNetCoreRuntimeVersion31Tests : DotNetCoreEndToEndTestsBase
    {
        ...
        [Fact]
        public async Task CanBuildAndRun_NetCore31WebApp_WhenOutputDirectoryIsCompressed()
        {
            // Arrange
            var dotnetcoreVersion = "3.1";
            var hostDir = Path.Combine(_hostSamplesDir, "DotNetCore", NetCoreApp31MvcApp);
            ...
            var buildImageScript = new ShellScriptBuilder()
               .AddCommand($"oryx build {appDir} --platform {DotNetCoreConstants.PlatformName} " +
               $"--platform-version {dotnetcoreVersion} -o {appOutputDir} --compress-destination-dir")
               .ToString();
            var runtimeImageScript = new ShellScriptBuilder()
                .AddCommand(
                $"oryx create-script -appPath {appOutputDir} -bindPort {ContainerPort}")
