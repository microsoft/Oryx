...
RUN apt-get update \
    ...
    && grep "node-v${NODE20_VERSION}-linux-x64.tar.xz" SHASUMS256.txt | sha256sum -c - \
    && mkdir -p /opt/nodejs \
    && tar -xJf "node-v${NODE20_VERSION}-linux-x64.tar.xz" -C /opt/nodejs --strip-components=1 \
    && rm -f "node-v${NODE20_VERSION}-linux-x64.tar.xz" SHASUMS256.txt SHASUMS256.txt.asc nodejs-keyring.kbx
