...
RUN pip install --upgrade pip \
    ...
    && pip install debugpy \
    && if [ "%PYTHON_MAJOR_VERSION%" = "3" ] && [ "%PYTHON_VERSION%" != "3.6" ] && [ "%PYTHON_VERSION%" != "3.11" ]; then pip install viztracer==0.14.3; fi\
    && if [ "%PYTHON_MAJOR_VERSION%" = "3" ] && [ "%PYTHON_VERSION%" != "3.6" ] && [ "%PYTHON_VERSION%" = "3.11" ]; then pip install viztracer==0.15.5; fi \
    && if [ "%PYTHON_MAJOR_VERSION%" = "3" ] && [ "%PYTHON_VERSION%" != "3.11" ]; then pip install vizplugins==0.1.2 \
