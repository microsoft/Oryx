...
RUN pip install --upgrade pip \
    ...
    && pip install debugpy \
    && if [ "3" = "3" ] && [ "3.8" != "3.6" ]; then pip install viztracer==0.14.3 \
    && pip install vizplugins==0.1.2 \
    && pip install orjson==3.6.6; fi \
    && if [ "3.8" = "3.7" ] || [ "3.8" = "3.8" ]; then curl -LO http://ftp.de.debian.org/debian/pool/main/libf/libffi/libffi6_3.2.1-9_amd64.deb \
