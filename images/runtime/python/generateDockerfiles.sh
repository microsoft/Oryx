...
do
    ...
    if [ "$MAJOR_MINOR_VERSION" == "2.7" ] || [ "$MAJOR_MINOR_VERSION" == "3.6" ] || \
       [ "$MAJOR_MINOR_VERSION" == "3.7" ] || [ "$MAJOR_MINOR_VERSION" == "3.8" ] || \
       [ "$MAJOR_MINOR_VERSION" == "3.9" ]
    then
        ORYX_PYTHON_IMAGE_BASE_TAG="oryx-run-base-$ImageDebianFlavor"
    else
        ORYX_PYTHON_IMAGE_BASE_TAG="python:$MAJOR_MINOR_VERSION-$ImageDebianFlavor"
    fi
    
    echo "$MAJOR_MINOR_VERSION" $ORYX_PYTHON_IMAGE_BASE_TAG
