...
do
	...
	DOTNET_DOWNLOAD_URL=$(echo $DOTNETCORE_PUBLIC_DOWNLOAD_URL_FORMAT | sed "s/#DOTNETVERSION#/\$NET_CORE_APP_${MAJOR_MINOR_VERSION}/g")
	ASPNET_DOWNLOAD_URL=$(echo $ASPNETCORE_PUBLIC_DOWNLOAD_URL_FORMAT | sed "s/#ASPNETVERSION#/\$ASPNET_CORE_APP_${MAJOR_MINOR_VERSION}/g")
	...
	if [[ " ${DOTNETCORE_STAGING_RUNTIME_VERSIONS[*]} " =~ " ${VERSION_DIRECTORY} " ]]; then
		...
		sed -i "s|$STAGING_DOTNET_DOWNLOAD_URL[[:space:]]\\\|$DOTNET_DOWNLOAD_URL \\\|g" "$TARGET_BASE_DOCKERFILE"
		sed -i "s|$STAGING_ASPNET_DOWNLOAD_URL[[:space:]]\\\|$ASPNET_DOWNLOAD_URL \\\|g" "$TARGET_BASE_DOCKERFILE"
	fi
done
