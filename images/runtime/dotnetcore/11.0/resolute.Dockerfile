...
FROM mcr.microsoft.com/dotnet/sdk:11.0-preview AS tools-install
ARG VSS_NUGET_URI_PREFIXES
ARG VSS_NUGET_EXTERNAL_FEED_ENDPOINTS
...
FROM mcr.microsoft.com/mirror/docker/library/ubuntu:resolute
ARG INCLUDE_AZURELINUX_CERTS
...
ARG NET_CORE_APP_110
ARG ASPNET_CORE_APP_110
...
RUN set -ex \
    ...
    && curl --fail --show-error --location \
        --remote-name https://builds.dotnet.microsoft.com/dotnet/Runtime/$NET_CORE_APP_110/dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz \
        --remote-name https://builds.dotnet.microsoft.com/dotnet/Runtime/$NET_CORE_APP_110/dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && sha512sum -c dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && mkdir -p /usr/share/dotnet \
    && tar -zxf dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz -C /usr/share/dotnet \
    && rm dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz \
    && rm dotnet-runtime-$NET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && ln -s /usr/share/dotnet/dotnet /usr/bin/dotnet \
    # Install ASP.NET Core
    && curl --fail --show-error --location \
        --remote-name https://builds.dotnet.microsoft.com/dotnet/aspnetcore/Runtime/$ASPNET_CORE_APP_110/aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz \
        --remote-name https://builds.dotnet.microsoft.com/dotnet/aspnetcore/Runtime/$ASPNET_CORE_APP_110/aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && sha512sum -c aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && mkdir -p /usr/share/dotnet \
    && tar -zxf aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz -C /usr/share/dotnet ./shared/Microsoft.AspNetCore.App \
    && rm aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz \
    && rm aspnetcore-runtime-$ASPNET_CORE_APP_110-linux-x64.tar.gz.sha512 \
    && dotnet-sos install \
...
RUN set -e; \
    ...
    rm -f /usr/local/bin/install-azurelinux-certs.sh; \
