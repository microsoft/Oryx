...
# based on resolution on https://github.com/NuGet/Announcements/issues/49#issue-795386700
    ...
    && curl -SL --output dotnet.tar.gz https://dotnetcli.blob.core.windows.net/dotnet-private/internal/$NET_CORE_APP_70/dotnet-runtime-$NET_CORE_APP_70-linux-x64.tar.gz$DOTNET_PRIVATE_STORAGE_ACCOUNT_ACCESS_TOKEN \
    ...
    && . ${BUILD_DIR}/__dotNetCoreRunTimeVersions.sh \
    && curl -SL --output aspnetcore.tar.gz https://dotnetcli.blob.core.windows.net/dotnet-private/internal/$ASPNET_CORE_APP_70/aspnetcore-runtime-$ASPNET_CORE_APP_70-linux-x64.tar.gz$DOTNET_PRIVATE_STORAGE_ACCOUNT_ACCESS_TOKEN \
