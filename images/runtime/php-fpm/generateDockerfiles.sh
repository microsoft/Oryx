...
do
	...
	PHP_IMAGE_NAME="php-fpm-$VERSION_DIRECTORY-$imageType"
	echo "Generating Dockerfile for image '$PHP_IMAGE_NAME' in directory '$VERSION_DIRECTORY'..."

	mkdir -p "$DIR/$VERSION_DIRECTORY/"
	TARGET_DOCKERFILE="$DIR/$VERSION_DIRECTORY/$imageType.Dockerfile"
	TARGET_DOCKERFILE_BASE="$DIR/$VERSION_DIRECTORY/base.$imageType.Dockerfile"
	...
	# RUNTIME_BASE_IMAGE_NAME="php-$VERSION_DIRECTORY-fpm-$PHP_FPM_RUNTIME_BASE_TAG-$ImageType"
    RUNTIME_BASE_IMAGE_NAME="mcr.microsoft.com/oryx/base:php-$VERSION_DIRECTORY-fpm-$PHP_FPM_RUNTIME_BASE_TAG-$imageType"
