...
then
	...
	docker tag "$builtImageTag" "$DOCKER_BUILD_IMAGE_REPO:latest"
	docker tag "$builtImageTag" "$DOCKER_BUILD_IMAGE_REPO:$uniqueTag"
	...
	echo "$DOCKER_BUILD_IMAGE_REPO:latest" > $BUILD_IMAGES_ARTIFACTS_FILE
	echo "$DOCKER_BUILD_IMAGE_REPO:$uniqueTag" >> $BUILD_IMAGES_ARTIFACTS_FILE
