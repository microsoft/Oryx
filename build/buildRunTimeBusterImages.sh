...
# Build the common base image for buster first, so other buster images that depend on it get the latest version.
...
dockerFiles=$(find $runtimeImagesSourceDir -type f \( -name "buster.Dockerfile" ! -path "$RUNTIME_IMAGES_SRC_DIR/commonbase/*" \) )
if [ -z "$dockerFiles" ]
then
...
for dockerFile in $dockerFiles; do
    ...
    localImageTagName="$ACR_PUBLIC_PREFIX/$getTagName_result-buster"
    ...
    then
        ...
        acrRuntimeImageTagNameRepo="$ACR_PUBLIC_PREFIX/$getTagName_result-buster"
        ...
        devBoxRuntimeImageTagNameRepo="$DEVBOX_RUNTIME_IMAGES_REPO_PREFIX/$getTagName_result-buster"
