...
if [ -n "$BUILD_NUMBER" ]; then
	...
	echo "$DOCKER_PACK_BUILDER_IMAGE_REPO:$BUILD_SUFFIX" >> $ACR_BUILD_IMAGES_ARTIFACTS_FILE

	docker tag "$DOCKER_PACK_IMAGE_REPO:latest" "$ACR_PACK_IMAGE_REPO:$BUILD_SUFFIX"
	echo "$DOCKER_PACK_IMAGE_REPO:$BUILD_SUFFIX" >> $ACR_BUILD_IMAGES_ARTIFACTS_FILE
fi
