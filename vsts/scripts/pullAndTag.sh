...
if [ -n "$TESTINTEGRATIONCASEFILTER" ];then
	...
	do
		...
		if [[ "$val" == "build-image="* ]];then
			...
			elif [[ "$strippedVal" == "cli-builder-debian-buster" ]];then
				buildImageFilter="cli"
				buildImageTagFilter="builder-debian-buster"
			fi
		fi
	done
fi
...
tagBuildImageForIntegrationTest "$imagefilter/cli" "builder-debian-buster" "$buildImageFilter" "$buildImageTagFilter"
